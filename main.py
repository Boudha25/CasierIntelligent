...
class LockerManagerGUI:
    def __init__(self, master, num_lockers):
        ...
        self.keypad_frame.grid(row=(num_lockers-1)//5 + 4, column=0, columnspan=5, pady=5)

        self.create_keypad()

        self.status_label = tk.Label(master, text="", wraplength=200)
        self.status_label.grid(row=(num_lockers-1)//5 + 5, columnspan=5, pady=5)
